...
float TemperaturaInterna()
{
    ...
    unsigned char entrada[9] = {0x01, 0x23, 0xC1, 0x06, 0x08, 0x06, 0x02};
    short crc = calcula_CRC(entrada, 7);

    union bytesCRC varreCRC;
    varreCRC.crc = crc;
    memcpy(&entrada[7], varreCRC.bytes, 2);

    if (uart0_filestream != -1)
    {
        int count = write(uart0_filestream, &entrada[0], 9);
        if (count < 0)
        {
...
float TemperaturaPotenciometro(){
   ...
   configuraUart();
    ...
    unsigned char entrada[9] = {0x01, 0x23, 0xC2, 0x06, 0x08, 0x06, 0x02};
    short crc = calcula_CRC(entrada, 7);
